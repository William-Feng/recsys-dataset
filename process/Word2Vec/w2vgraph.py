...
plt.scatter(
    x3, y3, label=f'Negative Sampling Exponent Correlation = {np.round(np.corrcoef(x1, y3)[0, 1], 2)}')

# Adding trend lines
fit1 = np.polyfit(x1, y1, 1)
plt.plot(x1, np.polyval(fit1, x1), linestyle='-', color='b',
         alpha=0.6, label='Window Size Trend Line')

fit2 = np.polyfit(x2, y2, 1)
plt.plot(x2, np.polyval(fit2, x2), linestyle='-',
         color='r', alpha=0.6, label='Epoch Trend Line')

fit3 = np.polyfit(x3, y3, 1)
plt.plot(x3, np.polyval(fit3, x3), linestyle='-', color='g',
         alpha=0.6, label='Negative Sampling Exponent Trend Line')
