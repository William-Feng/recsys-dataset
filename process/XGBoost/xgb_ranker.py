...
def make_predictions(rankers, wf, pipeline, weights):
    ...
    for ranker, weight in zip(rankers, weights):
        test_preds += weight * ranker.booster.predict(
            xgb.DMatrix(test_processed.compute())
        )

    test_data = test_data.with_columns(pl.Series(name="score", values=test_preds))

    test_predictions = (
        test_data.sort(["session", "score"], descending=True)
        .groupby("session")
        .agg([pl.col("aid").apply(list).alias("aid_list")])
    )
...
def create_submission_file(test_predictions, weights):
    ...
    for session, preds in zip(
        test_predictions["session"].to_numpy(), test_predictions["aid_list"].to_numpy()
    ):
    ...
    submission = submission.with_columns(
        [
            ...
            .apply(lambda x: int(x.split("_")[0]), return_dtype=pl.Int64)
            .alias("session_num"),
            submission["session_type"]
            .apply(lambda x: x.split("_")[1])
            .alias("session_action"),
        ]
    )

    submission = submission.with_columns(
        [
            submission["session_action"]
            .apply(lambda x: TYPE_LABELS[x])
            .alias("action_order")
        ]
    )
    submission = submission.sort(["session_num", "action_order"])
    submission = submission.drop(["session_num", "action_order", "session_action"])
